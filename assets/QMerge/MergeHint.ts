...
export default class MergeHint extends Component {
  ...
  static async teachCombine(hint: MergeHint, matches: MergeGrid[][]) {
    ...
    hint.startDrag(
      'Canvas/QMerge/MergeLayout/' + (grid[0].index + 1),
      'Canvas/QMerge/MergeLayout/' + (grid[1].index + 1)
    );
  ...
  static async teachDelete(hint: MergeHint, grid: MergeGrid) {
    ...
    trash.activate();
    hint.startDrag(
      'Canvas/QMerge/MergeLayout/' + (grid.index + 1),
      'Canvas/QMerge/Bottom/Trashcan'
    );
    ...
    evt
      .wait('QMerge.SwitchPosition', 10)
      .then(v => this.stopTeachDelete(hint, trash));
    evt
      .wait('Trashcan.Receive', 10)
      .then(v => this.stopTeachDelete(hint, trash));
  }
  ...
  static async check(hint: MergeHint) {
    ...
    if (matches.length > 0) {
      ...
      if (code == 'timeout') {
        ...
        if (matches.length > 0) await this.teachCombine(hint, matches);
      }
    } else {
      //没有合成的，且没有空格子
