import { _decorator, Component, Node, UITransformComponent, Vec2, Prefab, v3, Vec3, v2, instantiate } from "cc";
...
;
    ...
    onLoad() {
        ...
        this.slotLayoutNode.on(Node.EventType.TOUCH_CANCEL, this.onTouchCancel, this)
    ...
    fitToGridWithPos(entity: MergeEntity) {
        ...
        if (grid)
            this.moveToIndex(entity, grid.index);
    }
    ...
    onTouchBegan(e) {
        ...
        point = this.slotLayoutNode.transform.convertToNodeSpaceAR(v3(point.x, point.y, 0));
        let grid = this.grids.find(v => v.transform.getBoundingBox().contains(point))
    ...
    onTouchMoved(e) {
        ...
        if (this.drag_ent) {
            ...
            this.drag_ent.node.position = this.drag_ent.node.position.add3f(offset.x, offset.y, 0)
        }
    ...
    /**找空闲格子 */
    findEmptyGrid() {
        let grid = this.grids.find(v => !v.isLock && v.isUncover && v.entity == null)
        return grid;
    }
    ...
    doMerge() {
        ...
        if (matches.length > 0) {
            ...
            qanim.moveTo(this.drag_ent.node, 0.3, target.node.position, EaseType.sineOut).then(_ => {
                target.merge(drag);
                this.canOp = true;
    ...
    findMinLevelGrid() {
        ...
        if (min) {
            min = this.grids.filter(v => v.canDrag && v.entity).reduce((min, v) => v.entity.id < min.entity.id ? v : min, min)
        }
        return min;
    }
    ...
    findMaxLevelGrid() {
        ...
        if (max) {
            max = this.grids.filter(v => v.canDrag && v.entity).reduce((max, v) => v.entity.id > max.entity.id ? v : max, max
            )
        }
        return max;
    }
    ...
    addToGrid(grid: MergeGrid, id: number, noMsg: boolean = false): [AddRet, MergeEntity] {
        if (grid) {
            let obj = this.createMergeEntity(id, grid.index);
            grid.entity = obj;
            // add new entity 
            if (!noMsg)
                evt.emit(QMerge.Event.Add, grid);
    ...
    tagCancelFinished(drag_ent) {
        ...
        this.cancellingEntities[index].splice(this.cancellingEntities[index].indexOf(drag_ent), 1)
    }
    ...
    highlightAvaliable() {
        ...
        coms = coms.filter(v => v.id == this.drag_ent.id && v != this.drag_ent && v != this.curSelEnt)
        coms.forEach(v => {
    ...
    removeMergeEntity(ent: MergeEntity, removeNode = true) {
        if (ent) {
            ...
            if (grid.entity != null) {
                console.log(cc.js.formatStr("[QMerge]位置%s 移除id:%s", ent.index, ent.id));
            }
            if (removeNode)
                ent.del();
            grid.entity = null;
    ...
    onDragEnd(pos) {
        ...
        if (ent) {
            ...
            if (ent.isSameType(dragEnt)) {
                ...
                if (ok) {
                    console.log(cc.js.formatStr("[QMerge]位置：%s 合成Id:%s", ent.index, ent.id));
                    this.removeMergeEntity(dragEnt);
                    this.removeMergeEntity(this.curSelEnt);
    ...
    addNewBox(id: number, prefab_box: Prefab, boxType?: BoxQuality) {
        ...
        if (ret == MergePanel.AddRet.OK) {
            ...
            if (this.auto_open_delay > 0) {
                this.scheduleOnce(this.openBox.bind(this, box.index), this.auto_open_delay);
            }
            return true;
        }
        return false
