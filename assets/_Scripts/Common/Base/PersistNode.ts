...
export default class PersistNode extends Component {
  ...
  onViewShow(view) {
    ...
    if (
      csv.Config.BannerAdWhiteList &&
      csv.Config.BannerAdWhiteList.indexOf(view.node.name) != -1
    ) {
  ...
  onViewHidden(view) {
    ...
    if (
      csv.Config.BannerAdRefreshWhiteList &&
      csv.Config.BannerAdRefreshWhiteList.indexOf(view.node.name) != -1
    ) {
    ...
    if (ViewManager.instance) {
      ViewManager.instance.allViews.forEach(v => {
        ...
        if (v.node.active) {
          if (
            csv.Config.BannerAdWhiteList &&
            csv.Config.BannerAdWhiteList.indexOf(v.node.name) == -1
          ) {
            //没有在白名单里的要隐藏
