...
export default class FizzBody extends Component {
  @property({ type: Enum(FizzBodyType) })
  bodyType: FizzBodyType = FizzBodyType.Dynamic;

  @property({ type: Enum(FizzShapeType) })
  ...
  @property({
    visible() {
      return this.bodyType == FizzBodyType.Dynamic;
    },
    slide: true,
    range: [0, 10],
    step: 1,
  ...
  @property({
    visible() {
      return !this.isTrigger && this.shapeType == FizzShapeType.rect;
    },
  })
  left: boolean = false;
  @property({
  ...
  onLoad() {
    ...
    this._targetComponent = components.find(
      v => v['onFizzCollideEnter'] != null && v != this
    );
  }
  ...
  getDisplacement() {
    return v2(this.sx || 0, this.sy || 0);
  }

  get velocity() {
    return v2(this.xv || 0, this.yv || 0);
  }
  ...
  respawn() {
    if (!this._bodyAttached && FizzManager.instance) {
      ...
      if (this.bodyType == FizzBodyType.Dynamic) Fizz.setMass(this, 1);
  ...
  update(dt) {
    ...
    if (
      this.bodyType == FizzBodyType.Dynamic ||
      this.bodyType == FizzBodyType.Kinematic
    ) {
      // this.node.x = this.x;
      // this.node.y = this.y;
  ...
  isPathFinished() {
    return (
      ...
      this._currentPathIndex == this._runningPath.length
    );
  }
  ...
  followPath(
    ...
    stopAtEnd = false,
    maxSpeed = 100,
    pathRadius: number = 20,
    distDeacc: number = 100
  ) {
    ...
    if (distsq > pathRadius * pathRadius) {
      target = normalpoint.add(
        b
          .clone()
          .subtract(a)
          .normalize()
          .multiplyScalar(FizzBody.PathPredictLength + 10)
      );
