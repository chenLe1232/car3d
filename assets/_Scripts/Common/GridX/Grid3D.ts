...
export class Grid3D extends Component {
  ...
  @property({
    displayName: '起始偏移',
    tooltip: 'x,z通常为网格大小的一半，y会是子网格的一半',
  })
  ...
  onLoad() {
    ...
    this.tileSize = v3(
      this.size.x / this.mapSize.x,
      this.size.y / this.mapSize.y,
      this.size.z / this.mapSize.z
    );
  ...
  coordToPosition(coord: Vec3, anchor: Vec3 = v3(0.5, 0.5, 0.5)) {
    ...
    return v3(
      (coord.x + anchor.x) * this.tileSize.x - this.offset.x,
      (coord.y + anchor.y) * this.tileSize.y - this.offset.y,
      (coord.z + anchor.z) * this.tileSize.z - this.offset.z
    );
  }
  ...
  positionToCoord(localPos: Vec3) {
    ...
    // y 的偏移是一个子格子的长度
    this.tmp_tileCoord.x = Math.floor(
      (localPos.x + this.offset.x) / this.tileSize.x
    );
    this.tmp_tileCoord.z = Math.floor(
      (localPos.z + this.offset.z) / this.tileSize.z
    );
    this.tmp_tileCoord.y = Math.floor(
      (localPos.y + this.offset.y) / this.tileSize.y
    );
   ...
   */
  reuse(pos?: Vec3) {
    if (pos == null) pos = this.node.position;
    // need to be clear?
    this.clear();
    this.node.position = v3(
      pos.x * this.size.x + this.size.x / 2,
      pos.y * this.size.y + this.size.y / 2,
      pos.z * this.size.z + this.size.z / 2
    );
    this.pos.x = pos.x;
    this.pos.y = pos.z;
    this.pos.z = pos.z;
