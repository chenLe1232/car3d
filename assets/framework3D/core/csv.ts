...
csv.parse = function (csv_text: any) {
  ...
  csv_data.header = rows[csv.__HEADER_INDEX__]
    .replace(/[\r]/g, '')
    .split(/\s*\t/g); //.map(v=>v.replace(/\s/g,""))
  csv_data.type = rows[csv.__TYPE_INDEX__].replace(/[\r]/g, '').split(/\s*\t/g);
  csv_data.body = rows.slice(csv.__DATA_INDEX__).map(function (row) {
    return csv.parseRowWithT(row.replace(/\r/g, ''));
  });
