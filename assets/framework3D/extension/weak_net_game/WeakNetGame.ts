...
export default class WeakNetGame {
  ...
  static config: ServerConfig = {
    share_config_url: 'share_config.json',
    csv_url: 'csv/',
    csv: [],
    local_csv: [],
  };
  ...
  private static requestConfig() {
    ...
    return new Promise((resolve, reject) => {
      ...
      net
        .httpGet(this.serverConfig.config_url + '?r=' + Date.now())
        .then(res => {
  ...
  private static requestShareConfig() {
    ...
    return new Promise((resolve, reject) => {
      net
        .httpGet(
          this.serverConfig.cdn_url +
            this.config.share_config_url +
            '?t=' +
            Date.now()
        )
        .then(res => {
  ...
  static async login() {
    let params: any = { userId: this.serverConfig.userId };
    if (CC_WECHAT) {
      Platform.login();
    ...
    let res = await this.client.httpGet(
      this.serverConfig.root_url + '/game/login',
      params
    );
    if (res == Net.Code.Timeout) {
      ...
      if (res) {
        ...
        this.logined_useId = ret.data.userId;
        this.retry_count = 0;
  ...
  static async syncData(v, userId?) {
    if (userId == null) {
      if (WeakNetGame.isLoggedIn) {
        userId = this.logined_useId;
      } else {
        return console.warn('上传数据！，未登陆且未指定用户id');
      }
    }
    return this.syncDataToTable(v, 'user', userId);
  }
  ...
  static async syncDataToTable(v, table, id?) {
    ...
    let res = await this.client.httpPut(
      this.serverConfig.root_url + '/' + table + '/' + id,
      v
    );
    if (res == Net.Code.Timeout) {
      console.log('上传数据 超时....');
  ...
  static async loadUserInfo() {
    ...
    console.log('拉取用户数据');
    let res = await this.client.httpGet(
      this.serverConfig.root_url + '/user/' + this.logined_useId
    );
    if (res == Net.Code.Timeout) {
      return this.userInfo;
    } else {
  ...
  static async doLogin(progressCallback?): Promise<any> {
    if (!this.isInit) {
      ...
      if (!this.serverConfig.is_local_game) {
        ...
        if (this.serverConfig.is_login_enabled) {
  ...
  static doChoice(key: any, callback, target?, fail_callback?) {
    ...
    let shareToFriend = () => {
      ...
      evt.emit('wx.shareMessageToFriend');
    };
    let watch = () => {
      SovInfo.invoke(key);
    ...
    let share = (b = false) => {
      ...
      if (this.is_forbidden) {
        ...
        if (b) {
          ...
          Platform.doShareWithParams(
            { uuid, share_link: true },
            this.shareConfigs[key],
            () => {
          ...
          Platform.doShare(
            ...
            ok => {
              ...
              if (ok && notFail) {
                ...
                // fail_callback && fail_callback.call(target, 'share');
                if (CC_WECHAT) {
                  wx.showModal({
                    title: '提示',
