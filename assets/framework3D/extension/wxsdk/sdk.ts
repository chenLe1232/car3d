...
class WxSdk {
  ...
  public get parent() {
    ...
    if (info.scene == 1007 || info.scene == 1008) {
      //通过分享进入游戏
  ...
  public showShareMenu(cf: ShareInfo) {
    ...
    wx.onShareAppMessage(function () {
      let content = {
        title: GameConfig.default_share_title,
  ...
  public loadBannerAd(callback?) {
    ...
    let fixWidth = isPor
      ? this._systemInfo.screenWidth
      : this._systemInfo.screenHeight / 3;
    ...
    bannerAd.onError(err => {
      //加载失败
      console.log(
        'wxsdk bannerAd onError code:' + err.code + ' msg:' + err.msg
      );
