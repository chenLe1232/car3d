...
export default class List extends Component {
  ...
  _init() {
    ...
    if (!t.content) {
      error(t.node.name + "'s cc.ScrollViewComponent unset content!");
      return;
    }
    ...
    if (t.cyclic || 0) {
      t._ScrollViewComponent._processAutoScrolling =
    ...
    t.content.children.forEach((child: Node) => {
      ...
      if (child != t.tmpNode && child.isValid) child.destroy();
    });
    t._inited = true;
  }
  /**
  ...
  _processAutoScrolling(dt: number) {
    ...
    this._ScrollViewComponent['_autoScrollAccumulatedTime'] +=
      dt * (1 / brakingFactor);

    let percentage: number = Math.min(
      1,
      this._ScrollViewComponent['_autoScrollAccumulatedTime'] /
        this._ScrollViewComponent['_autoScrollTotalTime']
    );
    ...
    let newPosition: Vec3 = this._ScrollViewComponent[
      '_autoScrollStartPosition'
    ]
      .clone()
      .add(
        this._ScrollViewComponent['_autoScrollTargetDelta'].multiplyScalar(
          percentage
        )
      );
    let EPSILON: number =
      this._ScrollViewComponent['getScrollEndedEventTiming']();
    ...
    if (
      fireEvent &&
      !this._ScrollViewComponent['_isScrollEndedWithThresholdEventFired']
    ) {
      this._ScrollViewComponent['_dispatchEvent'](
    ...
    let deltaMove: any = newPosition.subtract(
      this._ScrollViewComponent.getContentPosition()
    );
    // cc.log(deltaMove)
    this._ScrollViewComponent['_moveContent'](
      this._ScrollViewComponent['_clampDelta'](deltaMove),
      reachedEnd
    );
  ...
  setTemplateItem(item: any) {
    ...
    else t._itemSize = size(item.width, item.height);
        ...
        case 1: //单行HORIZONTAL（LEFT_TO_RIGHT）、网格VERTICAL（LEFT_TO_RIGHT）
          if (scrollPos > -this._cyclicPos1) {
            ...
            if (this._ScrollViewComponent.isAutoScrolling()) {
              this._ScrollViewComponent['_autoScrollStartPosition'] =
                this._ScrollViewComponent['_autoScrollStartPosition'].subtract(
                  add
                );
            }
            // if (this._beganPos) {
            ...
            if (this._ScrollViewComponent.isAutoScrolling()) {
              this._ScrollViewComponent['_autoScrollStartPosition'] =
                this._ScrollViewComponent['_autoScrollStartPosition'].add(add);
            }
            // if (this._beganPos) {
            //     this._beganPos -= add;
            // }
  ...
  _onTouchCancelled(ev, captureListeners) {
    let t = this;
    if (
      t._ScrollViewComponent['_hasNestedViewGroup'](ev, captureListeners) ||
      ev.simulate
    )
      return;
  ...
  _updateListItem(listItem: ListItem) {
    ...
    if (this.selectedMode > SelectedType.NONE) {
      switch (this.selectedMode) {
        ...
        case SelectedType.MULT:
          listItem.selected =
            this.multSelected.indexOf(listItem.node['_listId']) >= 0;
          break;
      }
    }
  }
  ...
  _updateItemPos(listIdOrItem: any) {
    let item: Node = isNaN(listIdOrItem)
      ? listIdOrItem
  ...
  aniDelItem(listId: number, callFunc: Function, aniType: number) {
    ...
    if (!t.checkInited() || t.cyclic || !t._virtual)
      return error('This function is not allowed to be called!');

    if (t._aniDelRuning)
      return warn('Please wait for the current deletion to finish!');
  ...
  _calcNearestItem() {
    ...
    for (
      let n = 0;
      n < t.content.children.length && !breakFor;
      n += t._colLineNum
    ) {
  ...
  calcCustomSize(numItems: number) {
    let t: any = this;
    if (!t.checkInited()) return;
    if (!t._itemTmp) return error('Unset template item!');
    if (!t.renderEvent) return error('Unset Render-Event!');
    t._customSize = {};
