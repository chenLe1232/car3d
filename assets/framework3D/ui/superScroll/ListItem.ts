 ...
 * @end
 ******************************************/
const { ccclass, property, disallowMultiple, menu, executionOrder } =
  _decorator;
...
export default class ListItem extends Component {
  ...
  set selected(val: boolean) {
    ...
    switch (this.selectedMode) {
      ...
      case SelectedType.SWITCH:
        let sp: SpriteComponent =
          this.selectedFlag.getComponent(SpriteComponent);
        if (sp)
          sp.spriteFrame = val
  ...
  private _btnCom: any;
  get btnCom() {
    if (!this._btnCom) this._btnCom = this.node.getComponent(ButtonComponent);
    return this._btnCom;
  }
  //依赖的List组件
  public list: List;
  ...
  showAni(aniType: number, callFunc: Function, del: boolean) {
    ...
    switch (aniType) {
      case 0: //向上消失
        ...
        // ];
        tweener
          .to(0.2, { scale: scale(0.7) })
          .by(0.3, { position: v3(0, this.node.height * 2, 0) });
        break;
      case 1: //向右消失
        // acts = [
        //     cc.scaleTo(.2, .7),
        ...
        tweener
          .to(0.2, { scale: scale(0.7) })
          .by(0.3, { position: v3(this.node.width * 2, 0, 0) });
        break;
      case 2: //向下消失
        // acts = [
        //     cc.scaleTo(.2, .7),
      ...
      default: //默认：缩小消失
        ...
        //     cc.scaleTo(.3, .1),
        // ];
        tweener.to(0.3, { scale: scale(0.1) });
        break;
    }
    if (callFunc || del) {
      tweener.call(_ => {
