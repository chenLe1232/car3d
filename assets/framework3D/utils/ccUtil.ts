...
export default class ccUtil {
  ...
  public static playAnimation(
    ...
    if (stopAfter) {
      ...
      return new Promise(resolve => {
        anim.on(AnimationComponent.EventType.FINISHED, state => {
          //EventType
          resolve(state);
        });
      });
  ...
  static isGreaterDays(before: any, num = 7) {
    let now = new Date();
    var diff = now.getTime() - before;
    if (diff > 86400000 * num) {
      // 24*60*60*1000
      return true;
    }
  }

  static find<T extends Component>(
    path: string,
    node: Node,
    compType: { prototype: T }
  ): T {
    let n = find(path, node);
    if (n) {
  ...
  static setDisplay(sp: SpriteComponent, url: any, callback?: any) {
    if (typeof url == 'string') {
      ...
      if (url instanceof SpriteFrame) sp.spriteFrame = url;
    }
  }

  static getOrAddComponent<T extends Component>(
    obj: any,
    type: { prototype: T }
  ): T {
  ...
  static getComponentInParent<T extends Component>(
    obj: any,
    type: { prototype: T }
  ): T {
  ...
  static convertCameraWorldPosition(
    worldpos: any,
    cameraFrom: CameraComponent,
    cameraTo: CameraComponent
  ) {
    let pos = cameraFrom.worldToScreen(worldpos, Vec2.ZERO);
    let from = cameraTo.screenToWorld(pos, Vec2.ZERO);
    return from;
  }
  ...
  static setParent(node: any, newParent: any, keepWorldPosition = false) {
  ...
  public static getPrefab(path: any): Promise<Prefab> {
  ...
  public static getMaterial(path: any): Promise<Material> {
    return new Promise((resolve, reject) => {
      resources.load(path, Material, (err, res) => {
        if (err) return reject(err);
        resolve(res);
      });
    });
  ...
  //播放添加金币的动画
  static async playFlyCoin(
    template: Node | Prefab = null,
    parent: Node = null,
    from: Vec3 = display.center,
    to: Vec3 = display.leftTop,
    config?: FlyCoinConfig
  ) {
    if (config == null) {
