...
director.on(Director.EVENT_AFTER_SCENE_LAUNCH, () => {
  ...
  let allCanvases = director
    .getScene()
    ?.getComponentsInChildren(Canvas) as Canvas[];
  if (allCanvases.length <= 1) return;
  allCanvases = allCanvases.filter(x => !!x.cameraComponent);

  let allCameras = director
    .getScene()
    ?.getComponentsInChildren(Camera) as Camera[];
  let usedLayer = 0;
  ...
  persistCanvas.forEach(val => {
    const isLayerCollided = allCanvases.find(
      x =>
        x !== val &&
        x.cameraComponent!.visibility &
          val.cameraComponent!.visibility &
          customLayerMask
    );
    if (isLayerCollided) {
      ...
      val.cameraComponent!.visibility =
        lastAvailableLayer |
        (val.cameraComponent!.visibility & builtinLayerMask);
...
function getCanvasCameraLayer(node: Node) {
  ...
  if (canvas && canvas.cameraComponent) {
    if (canvas.cameraComponent.visibility & canvas.node.layer) {
      ...
      layer =
        canvas.cameraComponent.visibility &
        ~(canvas.cameraComponent.visibility - 1);
    }
    return layer;
  }
  if (node.parent) {
